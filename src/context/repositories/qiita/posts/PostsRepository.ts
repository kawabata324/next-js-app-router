...
export class PostsRepository {
  ...
  static async fetchPostsByUserId(userId: string): Promise<Posts> {
    const { QIITA_API_V2_URL } = BASE_URL
    const param = `page=${PostsRepository.MAX_PAGE_LENGTH}&per_page=${PostsRepository.MAX_POST_PER_PAGE}&query=qiita user:${userId}`
    const res = await fetch(`${QIITA_API_V2_URL}/items?${param}`, {
      headers: BaseHeaders.getQiitaBaseHeader(),
    })
    ...
    return PostsConvertor.convertPostsWithUser(posts)
  }
}
